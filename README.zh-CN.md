...
- **可视化与分析：** 支持实时边界电压、重构图像、固定 ROI 时序分析，以及由两个独立二维逆问题的成像结果经质量感知各向异性泛克里金得到的伪三维视图，输出同时带有逐节点相对方差。
...
录像同时纳入版本管理，位于 [`media/`](media/)，其中记录了每次实验的配置。伪三维视图由两层实测
结果经质量感知各向异性泛克里金得到：按各层质量加权，并校验实测平面保持不变，输出在数值之外还带有
相对方差。它仅使用两个独立二维参数场，不填造跨层电压，不等同于真实三维 CEM 反演，并且保留了分时
方案的层间时间差。图中电导率为相对各次实验已锁定参考的变化量，不是标定后的绝对值。
